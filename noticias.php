...
// configuración de paginación, solo 6 noticias por pag
$itemsPorPagina = 6;
$pagina = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
$inicio = ($pagina - 1) * $itemsPorPagina;

// Contar noticias
$totalQuery = $mysqli->query("SELECT COUNT(*) as total FROM Noticias");
$totalNoticias = $totalQuery->fetch_assoc()['total'];
$totalPaginas = ceil($totalNoticias / $itemsPorPagina);

// esta consulta obtiene las noticias ordenadas por la fecha de publi
$query = $mysqli->query("SELECT * FROM Noticias ORDER BY fecha_publicacion DESC LIMIT $inicio, $itemsPorPagina");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body class="bg-gray-50 text-gray-800 font-sans">
  ...
  <section class="py-16 px-6">
    <div class="max-w-7xl mx-auto">
      <h2 class="text-5xl font-extrabold text-center text-[#203474] mb-14">Noticias Recientes</h2>
        ...
        <?php while ($row = $query->fetch_assoc()): ?>
          <article class="bg-white rounded-3xl shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
            ...
            <div class="p-6 flex flex-col justify-between h-52">
              ...
              <a href="noticia.php?id=<?= $row['id_noticia'] ?>" 
      ...
      <div class="flex justify-center items-center gap-3 mt-12">
        <?php if ($pagina > 1): ?>
          <a href="?pagina=<?= $pagina-1 ?>" class="px-4 py-2 rounded-lg border border-[#203474] text-[#203474] hover:bg-[#203474] hover:text-white transition">
            ← Anterior
          </a>
        <?php endif; ?>
