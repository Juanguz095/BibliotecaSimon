...
// Datos del programa
$stmt = $mysqli->prepare("SELECT nombre, descripcionBreve, portada FROM Programas WHERE id_programa=?");
...
$categorias = $mysqli->query("SELECT id_categoriaPrograma, nombre 
                              FROM Categorias_Programas WHERE id_programa=$id_programa");

// Construir filtro de libros
$where = "id_categoriaPrograma IN (SELECT id_categoriaPrograma FROM Categorias_Programas WHERE id_programa=$id_programa)";

// filtro categoría
if (!empty($_GET['categoria'])) {
    $categoria = (int)$_GET['categoria'];
    $where .= " AND id_categoriaPrograma = $categoria";
}
...
// Libros del programa (todos o filtrados)
$libros = $mysqli->query("SELECT id_libro, titulo, descripcion, portada 
                          FROM Libros 
                          WHERE $where
                          LIMIT 30");
?>
...
<head>
  <meta charset="UTF-8">
  <title><?php echo $programa['nombre']; ?></title>
...
<body class="bg-gray-50 text-gray-800 font-sans min-h-screen flex flex-col">
  ...
  <section class="relative bg-gray-800 text-white py-28 mt-6 mx-4 rounded-2xl overflow-hidden">
    <img src="<?php echo $programa['portada']; ?>" 
         alt="Banner <?php echo $programa['nombre']; ?>" 
         class="absolute inset-0 w-full h-full object-cover opacity-60">

    <div class="relative z-10 text-center max-w-3xl mx-auto">
      <h1 class="text-5xl font-bold mb-4"><?php echo $programa['nombre']; ?></h1>
      <p class="text-lg"><?php echo $programa['descripcionBreve']; ?></p>
    </div>
  </section>

  <!-- Contenido con filtros y libros -->
  <section class="relative py-12 px-6">
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10">

      <!-- Filtros a la izquierda -->
      <aside class="md:col-span-1 bg-white rounded-2xl shadow-lg p-6 h-fit">
        <h2 class="text-2xl font-bold text-[#203474] mb-6 text-center">Filtrar Libros</h2>
        <form method="GET" action="programas_libros.php" class="space-y-6">
          ...
          <div>
            ...
            <select name="categoria" id="categoria" class="w-full border border-gray-300 rounded-lg px-4 py-2">
              <option value="">Todas</option>
              <?php 
              $categorias->data_seek(0);
              while($cat = $categorias->fetch_assoc()): ?>
                <option value="<?php echo $cat['id_categoriaPrograma']; ?>"
                  <?php if(isset($_GET['categoria']) && $_GET['categoria'] == $cat['id_categoriaPrograma']) echo "selected"; ?>>
                  <?php echo $cat['nombre']; ?>
                </option>
              <?php endwhile; ?>
            </select>
          ...
          <!-- Buscar -->
          <div>
            <label for="buscar" class="block text-sm font-medium text-gray-700 mb-2">Buscar título</label>
          ...
          <!-- Botón -->
          <button type="submit" 
                  class="w-full bg-[#203474] text-white px-6 py-3 rounded-lg font-bold hover:bg-[#4f6bbf] transition">
            Aplicar filtros
          </button>
        </form>
      </aside>
      ...
      <div class="md:col-span-3">
        <h2 class="text-3xl font-bold text-[#203474] mb-8 text-center md:text-left">Libros</h2>
        <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          <?php if ($libros->num_rows > 0): ?>
            <?php while($libro = $libros->fetch_assoc()): ?>
              <div class="group bg-white rounded-2xl shadow-lg overflow-hidden transform hover:-translate-y-2 hover:shadow-2xl transition flex flex-col">
                <img src="<?php echo $libro['portada']; ?>" 
                     alt="<?php echo $libro['titulo']; ?>" 
                     class="w-full h-56 object-cover group-hover:scale-105 transition duration-500">
                <div class="p-6 flex flex-col flex-1 justify-between">
                  <div>
                    <h3 class="text-xl font-bold text-[#203474] mb-3 group-hover:text-[#4f6bbf] transition">
                      <?php echo $libro['titulo']; ?>
                    </h3>
                  </div>
                  <a href="libro_prestamo.php?id=<?php echo $libro['id_libro']; ?>"
                    class="inline-block mt-auto px-5 py-2 bg-[#203474] text-white rounded-full font-bold shadow-md hover:shadow-lg hover:-translate-y-[2px] transition">
                    Ver libro
                  </a>
                </div>
              </div>
            <?php endwhile; ?>
          <?php else: ?>
            <p class="text-gray-500 text-center col-span-full">No se encontraron libros con estos filtros.</p>
          <?php endif; ?>
        </div>
      </div>
    </div>
