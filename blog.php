...
// Contar entradas del blog
$totalQuery = $mysqli->query("SELECT COUNT(*) as total FROM Blog");
$totalEntradas = $totalQuery->fetch_assoc()['total'];
$totalPaginas = ceil($totalEntradas / $itemsPorPagina);

// Obtener entradas actuales
$query = $mysqli->query("SELECT * FROM Blog ORDER BY fecha_publicacion DESC LIMIT $inicio, $itemsPorPagina");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body class="bg-gray-50 text-gray-800 font-sans">
  ...
  <!-- blog -->
  <section id="blog" class="bg-gray-50 py-16 px-6">
    <div class="max-w-7xl mx-auto">
      <h2 class="text-5xl font-extrabold text-center text-[#203474] mb-14">Entradas Recientes del Blog</h2>

      <!-- Grid -->
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php while ($row = $query->fetch_assoc()): ?>
          <article class="bg-white rounded-3xl shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
            ...
            <div class="p-6 flex flex-col justify-between h-52">
              <div>
                <h3 class="text-2xl font-extrabold text-[#203474] mb-2"><?= htmlspecialchars($row['titulo']) ?></h3>
                <p class="text-gray-600 text-base leading-relaxed">
                  <?= substr(strip_tags($row['contenido']), 0, 100) ?>...
                </p>
              </div>
              <a href="articulo_blog.php?id=<?= $row['id_blog'] ?>" 
                 class="mt-4 inline-block bg-[#203474] text-white px-5 py-2 rounded-xl font-semibold shadow hover:bg-[#16264f] transition">
                Leer más →
              </a>
            </div>
          </article>
      ...
      <div class="flex justify-center items-center gap-3 mt-12">
        <?php if ($pagina > 1): ?>
          <a href="?pagina=<?= $pagina-1 ?>" class="px-4 py-2 rounded-lg border border-[#203474] text-[#203474] hover:bg-[#203474] hover:text-white transition">← Anterior</a>
        <?php endif; ?>

        <?php for ($i=1; $i<=$totalPaginas; $i++): ?>
          <a href="?pagina=<?= $i ?>" 
             class="px-4 py-2 rounded-lg border border-[#203474] transition <?= $i==$pagina ? 'bg-[#203474] text-white' : 'text-[#203474] hover:bg-[#203474] hover:text-white' ?>">
            <?= $i ?>
          </a>
        <?php endfor; ?>

        <?php if ($pagina < $totalPaginas): ?>
          <a href="?pagina=<?= $pagina+1 ?>" class="px-4 py-2 rounded-lg border border-[#203474] text-[#203474] hover:bg-[#203474] hover:text-white transition">Siguiente →</a>
        <?php endif; ?>
      </div>
  ...
  <!-- FOOTER -->
  <footer id="contact" class="bg-[#292c3c] text-white py-10 px-4">
